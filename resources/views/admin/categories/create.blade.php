...
<div class="min-h-screen bg-gray-100 py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-900">{{ __('admin_categories.form.header_create') }}</h1>
                <p class="text-gray-600 mt-1">{{ __('admin_categories.subtitle') }}</p>
            </div>
            ...
            <form action="{{ route('admin.categories.store') }}" method="POST" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description"
                              ...
                              placeholder="{{ __('admin_categories.form.description_placeholder') }}">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex flex-col sm:flex-row sm:justify-end gap-3 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit"
                        ...
                        {{ __('admin_categories.form.create') }}
                    </button>
                </div>
            </form>
        </div>
